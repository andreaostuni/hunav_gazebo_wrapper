...
WorldGenerator::WorldGenerator() : Node("hunav_gazebo_world_generator")
{
  ...
  params_ = { ".id",          ".skin",         ".behavior",    ".group_id",    ".max_vel",
              ".radius",      ".init_pose.x",  ".init_pose.y", ".init_pose.z", ".init_pose.h",
              ".goal_radius", ".cyclic_goals", ".goals" };
...
void WorldGenerator::readAgentParams()
{
  ...
  for (std::string an : agent_names)
  {
    ...
    std::cout << "id: " << a.id << " skin:" << (int)a.skin << " behavior:" << (int)a.behavior
              << " group_id:" << (int)a.group_id << " max_vel:" << a.desired_velocity << " radius:" << a.radius
...
bool WorldGenerator::processXML()
{
  ...
  for (auto a : agents_.agents)
  {
    ...
    if (a.behavior == hunav_msgs::msg::Agent::BEH_REGULAR || a.behavior == hunav_msgs::msg::Agent::BEH_SURPRISED ||
